...
const public_Register_post = async (req, res) => {
  ...
  if (Grade ==="Grade1") {
    await User.findOne({Grade:Grade,Code:637275}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
      
    })
  }else if(Grade ==="Grade2"){
    await User.findOne({Grade:Grade,Code:774421}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
    })
  }else if(Grade ==="Grade3"){
    await User.findOne({Grade:Grade,Code:957307}).then((result)=>{
      quizesInfo = result.quizesInfo
      videosInfo = result.videosInfo
    })
  }
...
const forgetPassword_post = async(req,res)=>{
  ...
  try {
    ...
    if (!user&&phone) {
      ...
      const link  = `http://localhost:3333/reset-password/${user._id }/${token}`
   

        console.log("aerd",link)
...
const reset_password_post = async(req,res)=>{
  try {
    ...
    if (password1===password2) {
      ...
      await User.findByIdAndUpdate({_id:id},{Password:hashedPassword,PasswordNotHashed:password1}).then(()=>{
        res.redirect('/login')  
      }).catch((error)=>{
        res.send(error.message)
      })
